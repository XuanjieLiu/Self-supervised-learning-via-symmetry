...
import os
from codes.S3Ball.Batch_exp_KvsBatch.normal_rnn import Conv2dGruConv2d, LAST_CN_NUM, LAST_H, \
    LAST_W
...
class BallTrainer:
    def __init__(self, config, is_train=True):
        ...
        self.eval_data_loader = BallDataLoader(config['eval_data_path'], True)
    ...
    def train(self):
        ...
        epoch_num = self.train_data_loader.get_epoch_num_by_iter_and_batch_size(iter_num, self.batch_size)
        ...
        for i in range(iter_num, self.max_iter_num):
            curr_iter = iter_num
            data, new_epoch_num, progress = self.train_data_loader.load_a_batch_from_an_epoch(self.batch_size)
            ...
            T, Tr = make_translation_batch_2(batch_size=self.batch_size * self.t_batch_multiple, t_range=self.t_range)
            R, Rr, theta = make_rotation_Y_batch(batch_size=self.batch_size * self.r_batch_multiple,
