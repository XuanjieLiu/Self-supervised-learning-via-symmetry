...
import os
from codes.S3Ball.Dans_exp_singlColor.normal_rnn import Conv2dGruConv2d, BATCH_SIZE, LAST_CN_NUM, LAST_H, \
    LAST_W
from codes.S3Ball.ball_data_loader import BallDataLoader
from codes.S3Ball.symmetry import make_translation_batch, make_rotation_Y_batch, do_seq_symmetry, symm_trans, \
    symm_rotaY, make_translation_batch_2
...
class BallTrainer:
    ...
    def eval(self, epoch_num, iter_num, eval_loss_counter):
        ...
        data_shape = None
        label_list = []
        for data, trajectory in self.eval_data_loader.IterWithPosition(self.batch_size):
            trajectory = trajectory.to(DEVICE)
            label_list.append(trajectory)
            data = data.to(DEVICE)
        ...
        z_shape = tensor_z_gt.size()
        tensor_X = tensor_z_gt.reshape(z_shape[0] * z_shape[1] * z_shape[2], z_shape[3])
        tensor_Y = torch.stack(label_list).reshape(z_shape[0] * z_shape[1] * z_shape[2], z_shape[3])
        x_mse, y_mse, z_mse, xyz_mse = calc_group_mse(tensor_X, tensor_Y)
    ...
    def train(self):
        ...
        for i in range(iter_num, self.max_iter_num):
            ...
            T, Tr = make_translation_batch_2(batch_size=BATCH_SIZE * self.t_batch_multiple, t_range=self.t_range)
            ...
            rnn_loss = self.calc_rnn_loss(
                data[:, 1:, :, :, :], z_rpm, z0_rnn, recon_list
            ) if self.z_rnn_loss_scalar > 0 else torch.zeros(2)
            ...
            ) if self.t_batch_multiple else torch.zeros(2)
            ...
            ) if self.r_batch_multiple else torch.zeros(2)
            T_recon_loss = self.batch_symm_recon_loss(
                data[:, 1:, :, :, :], z_rpm,
                T_sample_points, self.t_batch_multiple,
                lambda z: symm_trans(z, T), lambda z: symm_trans(z, Tr)
            ) if self.t_batch_multiple else torch.zeros(1)[0]
            R_recon_loss = self.batch_symm_recon_loss(
                data[:, 1:, :, :, :], z_rpm,
                R_sample_points, self.r_batch_multiple,
                lambda z: symm_rotaY(z, R), lambda z: symm_rotaY(z, Rr)
            ) if self.r_batch_multiple else torch.zeros(1)[0]
