...
from codes.S3Ball.symmetry import make_translation_batch, make_rotation_Y_batch, do_seq_symmetry, symm_trans, \
    symm_rotaY, rand_translation, make_translation_batch_2
...
class BallTrainer:
    ...
    def eval(self, epoch_num, iter_num, eval_loss_counter):
        ...
        data_shape = None
        label_list = []
        for data, trajectory in self.eval_data_loader.IterWithPosition(BATCH_SIZE):
        ...
        tensor_X = tensor_z_gt.reshape(tensor_z_gt.size(0) * tensor_z_gt.size(1) * tensor_z_gt.size(2),
                                       tensor_z_gt.size(3))
